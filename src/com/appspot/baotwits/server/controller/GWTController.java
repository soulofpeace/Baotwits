...
import com.appspot.baotwits.server.service.util.ServiceUtils;
...
public class GWTController extends RemoteServiceServlet implements Controller, ServletContextAware {
	...
	private void preprocessHTTP(HttpServletRequest request, HttpServletResponse response){
		ServiceUtils.disableResponseCaching(response);
	}
	...
	public ModelAndView handleRequest(HttpServletRequest request,
		...
		try{
			...
			ServiceUtils.setRequest(request);
			ServiceUtils.setResponse(response);
			doPost(request, response);
		}finally{
			ServiceUtils.setRequest(null);
			ServiceUtils.setResponse(null);
		}
